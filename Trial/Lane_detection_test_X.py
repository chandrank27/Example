...
def detect_line_segments(cropped_edges):
    # tuning min_threshold, minLineLength, maxLineGap is a trial and error process
...
def detect_line_segments(cropped_edges):
    ...
    line_segments = cv2.HoughLinesP(cropped_edges, rho, theta, min_threshold, 
                                    np.array([]), minLineLength=3, maxLineGap=0)
...
def average_slope_intercept(frame, line_segments):
    ...
    if len(central_left_fit) > 0:
        ...
        #print(left_fit_average)
        central_lane_lines.append(make_points(frame, central_left_fit_average))

    if len(central_right_fit) > 0:
        central_right_fit_average = np.average(central_right_fit, axis=0)
        #print(right_fit_average)
        central_lane_lines.append(make_points(frame, central_right_fit_average))
